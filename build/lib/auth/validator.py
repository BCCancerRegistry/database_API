...
from BCCancerAPI.dbmodels import get_rw_eng
...
from BCCancerAPI.auth import JWTBearer
env = Environ()
...
def get_user(username: str):
    ...
    for i,j in fake_users_db.items():
        results=[]
        if i==username:
            results.append(j)
    if len(results) == 1:
        return results[0]#user_to_dict(results[0])
...
async def protected_route(token: str = Depends(oauth2_scheme)):
    try:
        ...
        if payload.get("profile")=="Admin":
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="User doesnt have priviledges  credentials",
                headers={"WWW-Authenticate": "Bearer"},
            )
